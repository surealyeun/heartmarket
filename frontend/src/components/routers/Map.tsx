...
class Map extends Component<any> {
  ...
  getTrade = async (area: string) => {
    await fetch('http://13.125.55.96:8080/trade/search/area/' + area + '?no=' + this.state.no, {
      method: 'GET'
    })
      .then(res => res.json())
      .then(json => {
  ...
  rendList = (area: string) => {
    ...
    let content = '<div class="info">' + '<div class="title">' + area + '</div><div class="size">'
    ...
    for (let i = 0; i < trade.length; i++) {
      ...
      if (trade[i].ttitle.length > 0) {
        content += `<a href=${tradeNo}><div class="Map_post"><div><span class="Map_title">게시글: ${trade[i].ttitle.substring(0, 20)}</span>가격 : ${
          trade[i].pprice
        }</div><div class="Map_post_container"><img class="Map_post_img" src=${trade[i].tlist}></img></div></div></a>`
      }
