...
class Map extends Component<any> {
  ...
  getTrade = async (area: string) => {
    await fetch(
      'http://13.125.55.96:8080/trade/search/area/' +
        area +
        '?no=' +
        this.state.no,
      {
      ...
      .then(json => {
        ...
        if (data != null) {
          this.setState(() => {
            return { trade: data, curarea: area };
          });
        } else {
          this.setState(() => {
            return { trade: this.default, curarea: area };
          });
        }
  ...
  rendList = (area: string) => {
    ...
    for (let i = 0; i < trade.length; i++) {
      ...
      if (trade[i].ttitle.length > 0) {
        content += `<a href=${tradeNo}><div class="Map_post"><div><span class="Map_title">게시글: ${trade[i]
          .ttitle.substring(0, 20)}</span>가격 : ${trade[i].pprice}</div><div class="Map_post_container"><img class="Map_post_img" src=${trade[i].tlist}/></div></div></a>`;
      }
  ...
  componentDidMount() {
    ...
    function displayMarker(place: any) {
      ...
      for (let i = 0; i < place.path.length; i++) {
        bounds.extend(
          new window.kakao.maps.LatLng(place.path[i].Ha, place.path[i].Ga),
        );
      }
  ...
  handlePrePage = async () => {
    if (this.state.no > 0) {
      ...
      await this.getTrade(this.state.curarea);
      console.log(this.state.no);
      this.rendList(this.state.curarea);
    }
  };
    ...
    return (
      <div className="map">
        <div className="map_wrap">
          ...
          >
            <div className="hAddr">
              <button className="previous_page" onClick={this.handleOnClick}>
                뒤로가기
              </button>
              ...
              <br />
              <div className="page">
                <button onClick={this.handlePrePage} className="btn">
                  {"<"}
                </button>
